...
/**
 * Owns the source-scan viewer's open/close lifecycle and the pan/zoom
 * transform (drag, wheel, double-tap reset, expand-from-thumbnail intro
 * animation). Fully independent of the annotation/drawing state.
 */
export function useScanViewport() {
  ...
  const setNaturalSize = useCallback((width: number, height: number) => {
    dispatchViewer({ type: "set_natural_size", width, height });
  }, []);
  ...
  const resetSourceScan = useCallback(() => {
  ...
  }, [getDefaultScanTransform]);

  // Callers should skip invoking this while a drawing tool is active — the
  // SVG layer handles input in that case, never pan.
  ...
  const zoomSourceScan = useCallback(
    (factor: number) => {
      dispatchViewer({ type: "settle_start" });
  ...
  return {
    ...
    scanTransform,
    scanFitSize,
    imagePreviewButtonRef,
    scanViewportRef,
    imageHasDraggedRef,
    handleOpenImage,
    handleCloseImage,
    handleScanPointerDown,
    handleScanPointerMove,
    handleScanPointerUp,
    zoomSourceScan,
    resetSourceScan,
    setNaturalSize,
